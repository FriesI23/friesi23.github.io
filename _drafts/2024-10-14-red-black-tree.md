...
将 y 节点的值替换 z 节点的值, 然后准备删除 y (此时已经回到了情况1).
...
通过上述操作, 我们可以得出, `二叉 b-树` 咋随机查询, 插入和删除时最坏的情况下与树的高度成比例,
...
在二叉 B-树中, 我们使用 `δ-level` 来表示其层级. 根据性质2, `δ-level` 对于每一条到叶子节点的路径都是相同的.
这其实就是红黑树中最后一条性质: "对于每一个节点, 从该节点到期所有节点的简单路径上, 均包含相同数量的黑色节点".

而被 `δ-pointer` 指向的节点就是 `RED` 节点, 否则为 `BLACK` 节点. 这就对应了红黑树中的性质一;
而 ROOT 节点必定不会被 `δ-pointer` 指向, 因此满足红黑树性质2, 即 ROOT 一定为黑色节点.

而不存在连续 `σ-pointer` 便是红黑树中的性质4, 即一个红色节点的两个子节点都是黑色的.

最后, 我们将所有最低 `δ-level` 上的所有被 `σ-pointer` 连接的叶子节点都赋予一个 `BLACK` 的 NIL 节点, 即可满足红黑树性质3.
...
其实有了上面两小节的铺垫, 红黑树就很好理解了. 因为红黑树和 `对称二叉 B-树`, `二叉 B-树` 本质上就是一种东西!

我们可以这么说, `Binary B-Tree` 本质上是 `2-3 Tree` 的一种等价形式.
而从[小结](#二叉-b-树小结)得知, `Binary B-Tree` 其实就是 `Red-Black Tree` 中 `RED` 节点永远置于节点右侧的特例.
而 `Symmetric Binary B-tree` 平衡了 `Binary B-Tree`, 其本质是 `2-3-4 Tree` 的一种等价形式.
也就是说, `Symmetric Binary B-tree` 和 `Red-Black Tree` 从逻辑上就是一个东西, 只是从概念上进行了一定区分.
比如 `σ-arcs` 与 `δ-level` 其实就是红黑树中的红色节点 (RED) 和黑高 (Black Height).
...
| 性质1 | 一个节点只能是红色 (RED) 或者 (BLACK) 的                                        | 一个节点只能被 "水平 (horizontal)" (`σ-pointer`) 与 "垂直 (vertical)" (`δ-pointer`) 两种指针中的一种指向       |
| 性质2 | 根节点是黑色 (BLACK) 的                                                         | 根节点肯定不可能是被 `σ-pointer` 指向的                                                                        |
| 性质3 | 每个叶子节点 (NIL) 是黑色 (BLACK) 的                                            | 所有叶子节点都在同一 `δ-level` (不需要红黑树中的 NIL 节点填充, 已经足够表达算法性质)                           |
| 性质4 | 如果一个节点是红的 (RED), 那么其两个子节点都是黑的 (BLACK)                      | 不存在连续被 `σ-pointer` 指向的节点链. (e.g. `x1 -> x2 [-> x3] (INVALID)`)                                     |
| 性质5 | 对于每个节点, 该节点到后代叶节点的简单路径上, 均包含相同数量的黑色 (BLACK) 节点 | 所有叶子节点都在同一 `δ-level` (这代表对于每个节点, 该节点到后代叶节点的简单路径上, 均包含相同数量的 `δ-arcs`) |
...
树高度也很好得出: `lg(N+1) <= h <= 2lg(N+2)-2`, N 为节点数量. 当然也可以粗略的估计 `h <= 2lg(N+1)`.

下面将列举主要操作并进行说明.

### 红黑树插入 (Insert)

首先我们需要在叶子节点处插入一个红色节点, 也就是在 `对称二叉 B-树` 的叶子节点处水平插入一个节点 z, 其父节点为 y.
插入完成后, 我们便要考察 y 的颜色, 已确定是否违反红黑树的性质:

```text
// 我将对称二叉 B-树放在了前面, 以便比较两者的关系
...
// 只列出一侧的情况, 另一侧同理 (镜像)
  ...
  x              -->      x                    x                  x
    \                      \                    \               /   \
      y (RED)               y (RED)              y (RED)       z     y
                             \                  /
                              z (RED)          z (RED)
```

对于 y 为黑色的情况, 由于没有破坏红黑树性质, 因此直接插入结束.
而对于双红色的情况, 我们需要递归的进行修复进行修复.

这里再次祭出 `对称二叉 B-树` 插入修复的流程, 同时附上红黑树的等价关系:
